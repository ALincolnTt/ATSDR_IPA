...
groupTitle <- "Distribution of uncertainty around the mean"
generic.caption <- paste0(format(numsamps, big.mark = ","), " draws")
blankPlot <- ggplot2::ggplot() + ggplot2::geom_blank(ggplot2::aes(1,1)) +
    ggplot2::theme_void()
tg <- grid::textGrob(groupTitle, gp=grid::gpar(fontsize=12, fontface="bold"))
...
```{r pod.fm.lung, out.width="75%"}

# Graph POD for female mice, lung tumors
...
pdf_pod.fm.lung <- ggplot2::ggplot(pod.fm.lung, ggplot2::aes(EC.fm.lung)) + 
    ggplot2::geom_histogram(bins=200, color=pal6[3], fill=pal6[3]
                            , alpha=0.5) +
    ggplot2::xlim(5000,40000) +
    ggplot2::theme_bw() +
...
fn_pod.fm.lung <- file.path(outdir,"Hist_POD_fm_lung.png")
ggplot2::ggsave(fn_pod.fm.lung, pdf_pod.fm.lung, height=5, width=7
                , dpi=bestdpi)

knitr::include_graphics(path = fn_pod.fm.lung)

```

Unit risk can be calculated by dividing the benchmark response rate by the POD value:

```{r ur.fm.lung, out.width="75%"}

# Graph IUR for female mice, lung tumors
BMR <- 0.1 # Benchmark response rate
ur.fm.lung <- BMR / pod.fm.lung$EC.fm.lung
ur.fm.lung <- as.data.frame(ur.fm.lung)
colnames(ur.fm.lung)[1] <- "ur.fm.lung"
pdf_ur.fm.lung <- ggplot2::ggplot(ur.fm.lung, ggplot2::aes(ur.fm.lung)) + 
    ggplot2::geom_histogram(bins=200,color=pal6[3],fill=pal6[3],alpha=0.5) +
    # ggplot2::xlim(1e-06,1.4e-05) +
    ggplot2::scale_x_continuous(name=NULL,breaks=c(1e-6,3e-6,5e-6,7e-6,9e-6
                                                   , 1.1e-5, 1.3e-5, 1.5e-5)) +
    ggplot2::theme_bw() +
    ggplot2::labs(title="Distribution of uncertainty around the mean"
...
fn_ur.fm.lung <- file.path(outdir,"Hist_IUR_fm_lung.png")
ggplot2::ggsave(fn_ur.fm.lung, pdf_ur.fm.lung, height=5, width=7, dpi=bestdpi)

knitr::include_graphics(path = fn_ur.fm.lung)
...
```{r fm.lung.alt, out.width="75%"}

# Graph alternate POD for female mice, lung tumors
...
pdf_pod.fm.lung.alt <- ggplot2::ggplot(pod.fm.lung.alt
                                       , ggplot2::aes(EC.fm.lung.alt)) + 
    ggplot2::geom_histogram(bins=200, color=pal6[5], fill=pal6[5], alpha=0.5) +
    ggplot2::xlim(5000,40000) +
    ggplot2::theme_bw() +
...
fn_pdf_pod.fm.lung.alt <- file.path(outdir,"Hist_POD_fm_lung_alt.png")
ggplot2::ggsave(fn_pdf_pod.fm.lung.alt, pdf_pod.fm.lung.alt, height=5, width=7
                , dpi=bestdpi)

knitr::include_graphics(path = fn_pdf_pod.fm.lung.alt)

# Graph alternate IUR for female mice, lung tumors
...
pdf_ur.fm.lung.alt <- ggplot2::ggplot(ur.fm.lung.alt
                                      , ggplot2::aes(ur.fm.lung.alt)) + 
    ggplot2::geom_histogram(bins=200,color=pal6[5],fill=pal6[5],alpha=0.5) +
    ggplot2::xlim(1e-06,1.4e-05) +
    ggplot2::scale_x_continuous(name=NULL,breaks=c(1e-6,3e-6,5e-6,7e-6,9e-6
                                                   , 1.1e-5, 1.3e-5, 1.5e-5)) +
...
fn_pdf_ur.fm.lung.alt <- file.path(outdir,"Hist_IUR_fm_lung_alt.png")
ggplot2::ggsave(fn_pdf_ur.fm.lung.alt, pdf_ur.fm.lung.alt, height=5, width=7
                , dpi=bestdpi)

pod.fm.lymph.alt <- EnvStats::rtri(numsamps, 11100, 52800, 21100)
ur.fm.lymph.alt <- data.frame("ur.fm.lymph.alt" = BMR / pod.fm.lymph.alt)

pod.fm.uter.alt <- EnvStats::rtri(numsamps, 18500, 82000, 32800)
ur.fm.uter.alt <- data.frame("ur.fm.uter.alt" = BMR / pod.fm.uter.alt)

pod.fm.mamm.alt <- EnvStats::rtri(numsamps, 4280, 24200, 9690)
ur.fm.mamm.alt <- data.frame("ur.fm.mamm.alt" = BMR / pod.fm.mamm.alt)

ur.fm.comb.alt <- ur.fm.lung.alt + ur.fm.lymph.alt + ur.fm.uter.alt + ur.fm.mamm.alt
ur.fm.comb.alt <- as.data.frame(ur.fm.comb.alt)
colnames(ur.fm.comb.alt)[1] <- "UnitRisk"

knitr::include_graphics(path = fn_pdf_ur.fm.lung.alt)
...
```{r ur.fm.comb1, fig.show="hide"}

# Generate POD and IUR for female mice, different tumor types
# Lung tumors
pod.fm.lung <- EnvStats::rtri(numsamps, 9100, 34000, 14800)
pod.fm.lung <- as.data.frame(pod.fm.lung)
...
# Generate ECDF for combined tumor types
cdf <- ggplot2::ggplot(ur.fm.comb, ggplot2::aes(UnitRisk)) +
    ggplot2::stat_ecdf(geom = "point", color=pal6[3], size=1) +
    ggplot2::xlim(1e-05,4e-05) +
    ggplot2::theme_bw() +
...
ur.fm.all.subtitle <- paste("Screening-level unit risk, female mice"
                        , "lung, lymph, uterine, and mammary tumors"
                        , sep = "\n")
stg <- grid::textGrob(ur.fm.all.subtitle, gp=grid::gpar(fontsize=10))
cap <- grid::textGrob(generic.caption, gp=grid::gpar(fontsize=8
                                                     , fontface="italic"))
    
fn_ur.fm.comb <- file.path(outdir,"Hist_ECDF_UR_FM_ALL.png")
p_ur.fm.comb <- gridExtra::grid.arrange(tg, stg, pdf, blankPlot, cdf, cap
                            , ncol=4, nrow=4
                            , layout_matrix = rbind(c(1,1,1,1), c(2,2,2,2)
                                                    , c(3,4,5,4), c(6,6,6,6))
                            , heights = c(0.2, 0.3, 4, 0.2)
                            , widths = c(3.5,0.1,3.5,0.1))

```

```{r display_ur.fm.comb, out.width="75%"}

ggplot2::ggsave(fn_ur.fm.comb, p_ur.fm.comb, height = 6, width = 8
                , dpi=bestdpi)

knitr::include_graphics(path = fn_ur.fm.comb)
...
```{r bdms_example, fig.show="hide"}

# Read BMDS output data
...
# Generate PDF for female mouse lung tumors (refined)
pdf2 <- ggplot2::ggplot(df_ur.fm.lung2, ggplot2::aes(FM.Lung)) + 
    ggplot2::geom_density(ggplot2::aes(y=ggplot2::after_stat(count/max(count)))
                        , color=pal6[1], size=1) +
    ggplot2::xlim(4e-06,2e-05) +
    ggplot2::theme_bw() +
    ggplot2::labs(title="Probability Density Function"
                  , x = xlabel_ur
                  , y = "Density") +
...
# Generate ECDF for female mouse lung tumors (refined)
cdf2 <- ggplot2::ggplot(df_ur.fm.lung2, ggplot2::aes(FM.Lung)) +
    ggplot2::stat_ecdf(geom = "point", color=pal6[1], size=1) +
    ggplot2::xlim(4e-06,2e-05) +
    ggplot2::theme_bw() +
    ggplot2::labs(title="Empirical CDF"
                  , x = xlabel_ur
...
# Combine plots into one figure
ur.fm.ref.lung.subtitle <- "Refined unit risk, female mice, lung tumors"
BMDScaption <- paste(generic.caption,"from BMDS output report", sep = " ")
stg <- grid::textGrob(ur.fm.ref.lung.subtitle, gp=grid::gpar(fontsize=10))
cap <- grid::textGrob(BMDScaption, gp=grid::gpar(fontsize=8
                                                     , fontface="italic"))
    
fn.fm.lung.ur <- file.path(outdir,"Hist_ECDF_iur_fm_lung.png")
p.fm.lung.ur <- gridExtra::grid.arrange(tg, stg, pdf2, blankPlot, cdf2, cap
                            , ncol=4, nrow=4
                            , layout_matrix = rbind(c(1,1,1,1), c(2,2,2,2)
                                                    , c(3,4,5,4), c(6,6,6,6))
                            , heights = c(0.2, 0.2, 4, 0.2)
                            , widths = c(3.5,0.1,3.5,0.1))

```

```{r display_fm.lung.ur, out.width="75%"}

ggplot2::ggsave(fn.fm.lung.ur, p.fm.lung.ur, height = 6, width = 8
                , dpi=bestdpi)

knitr::include_graphics(path = fn.fm.lung.ur)
...
```{r ur.comb.BMDSdata, fig.show='hide'}

# Sample additional tumor types and combine
...
# Combine plots into one figure
ur.fm.ref.subtitle <- paste("Refined unit risk, female mice"
                            , "lung, lymph, uterine, and mammary tumors", sep="\n")
stg <- grid::textGrob(ur.fm.ref.subtitle, gp=grid::gpar(fontsize=10))
cap <- grid::textGrob(BMDScaption, gp=grid::gpar(fontsize=8
                                                 , fontface="italic"))
    
fn.PDFCDF.fm.all.ur <- file.path(outdir,"Hist_ECDF_iur_fm_all.png")
p_PDF_CDF_iur.fm.all <- gridExtra::grid.arrange(tg, stg, pdf3, blankPlot, cdf3
                            , cap, ncol=4, nrow=4
                            , layout_matrix = rbind(c(1,1,1,1), c(2,2,2,2)
                                                    , c(3,4,5,4), c(6,6,6,6))
                            , heights = c(0.2, 0.3, 4, 0.2)
...
```{r display_ur.fm.comb2, out.width="75%"}

ggplot2::ggsave(fn.PDFCDF.fm.all.ur, p_PDF_CDF_iur.fm.all, height = 6
                , width = 8, dpi=bestdpi)

knitr::include_graphics(path = fn.PDFCDF.fm.all.ur)

```

####Compare Mouse Screening/Refined IURs

```{r mouseCompScreenRefined, out.width="75%"}

# Compare Screening-level IUR to Refined IUR
ur.fm.comb$Type <- "Screening-level, 90% CI"
ur.fm.comb2$Type <- "Refined"
ur.fm.comb.alt$Type <- "Screening-level, alternate"
ur.fm.comb2 <- ur.fm.comb2[!is.na(ur.fm.comb2$UnitRisk),]
ur.fm.comb3 <- rbind(ur.fm.comb, ur.fm.comb.alt, ur.fm.comb2)

# Calculate required percentiles
screen99 <- signif(quantile(ur.fm.comb$UnitRisk, probs = 0.99), digits=3)
refine95 <- signif(quantile(ur.fm.comb2$UnitRisk, probs=0.95), digits=3)
screen99alt <- signif(quantile(ur.fm.comb.alt$UnitRisk, probs=0.99), digits=3)
...
pdf_mcomp <- ggplot2::ggplot(ur.fm.comb3
    ...
    # ggplot2::xlim(1e-05,6e-05) +
    ggplot2::scale_color_manual(values=c(pal6[1], pal6[3],pal6[5])) +
    ggplot2::scale_linetype_manual(values=c("solid", "solid", "dashed")) +
    ggplot2::theme_bw() +
    ggplot2::labs(title=groupTitle
...
ggplot2::ggsave(fn_mcomp, pdf_mcomp,height=4, width=5, units="in", dpi=bestdpi)
...
rm(BMR, cdf, cdf2, cdf3, df_pods2, df_urs2, df_ur.fm.lung2, pdf, pdf2, pdf3
   ...
   , ur.fm.lymph, ur.fm.lymph2, ur.fm.mamm, ur.fm.mam2, ur.fm.uter
   , ur.fm.uter2, BMDScaption, fn, fn.fm.lung.ur, fn.PDFCDF.fm.all.ur
   , fn_mcomp, fn_pdf_pod.fm.lung.alt, fn_pdf_ur.fm.lung.alt, fn_pod.fm.lung
   , fn_ur.fm.comb, fn_ur.fm.lung, meanBMDS, meanCombIUR, medBMDS, medCombIUR
   , p.fm.lung.ur, p_PDF_CDF_iur.fm.all, p_ur.fm.comb, ur.fm.all.subtitle
   , ur.fm.comb3, ur.fm.lung.subtitle, pdf_mcomp, pdf_pod.fm.lung
   , pdf_pod.fm.lung.alt, pdf_ur.fm.lung, pdf_ur.fm.lung.alt)
...
```{r ur.epi.lymph, out.width="75%"}

# Set conversion factor for EtO from ppm to ug/m3
CF_ppm_ugm3 <- 1802

# Generate EC ug/m3 distribution by converting distribution (use this one)
pod.epi.lymph.ugm3 <- as.data.frame(EnvStats::rtri(numsamps, 0.00271, 0.0663,
                                               0.0107)*CF_ppm_ugm3)
colnames(pod.epi.lymph.ugm3)[1] <- "EC_ug_m3"

# Set BenchMark Response rate for epidemiological studies
BMRepi <- 0.01
ur.epi.lymph.perugm3 <- dplyr::mutate(pod.epi.lymph.ugm3
                              , UnitRisk = eval(BMRepi) / EC_ug_m3)

# Generate PDF for breast cancers
pdf_ur.epi.lymph <- ggplot2::ggplot(ur.epi.lymph.perugm3
                                , ggplot2::aes(UnitRisk)) + 
    ggplot2::geom_density(ggplot2::aes(y=ggplot2::after_stat(count/max(count)))
                          , color=pal6[3], size=1) +
    ggplot2::xlim(0,0.002) +
    ggplot2::theme_bw() +
    ggplot2::labs(subtitle="Lymphoid cancers in workers"
                  , y = "Density") +
...
# fn_ur.epi.lymph <- file.path(outdir,"Hist_UR_epi_lymph.png")
# ggplot2::ggsave(fn_ur.epi.lymph, pdf_ur.epi.lymph, height=5, width=7, dpi=600)
# 
# knitr::include_graphics(path = fn_ur.epi.lymph)
...
```{r ur.epi.comb, fig.show="hide"}

# Breast cancers
pod.epi.breast.ugm3 <- as.data.frame(EnvStats::rtri(numsamps, 0.0101, 0.128
                                               , 0.0206)*CF_ppm_ugm3)
...
pdf_ur.epi.breast <- ggplot2::ggplot(ur.epi.breast.perugm3
    ...
    ggplot2::xlim(0,0.002) +
    ggplot2::theme_bw() +
    ggplot2::labs(subtitle="Breast cancer in workers"
                  , y = "Density") +
    ggplot2::theme(plot.subtitle = ggplot2::element_text(hjust=0.5, size=8)
                   , axis.text = ggplot2::element_text(size=6)
                   , axis.title.x = ggplot2::element_blank()
...
# fn_ur.epi.breast <- file.path(outdir,"Hist_UR_epi_breast.png")
# ggplot2::ggsave(fn_ur.epi.breast, pdf_ur.epi.breast, height=5, width=7
#                 , dpi=600)
# 
# knitr::include_graphics(path = fn_ur.epi.breast)
...
epi_caption <- paste0("All cancers: Median=", round(median.epi.comb,5)
                   , "; 95th percentile=", round(ucl.epi.comb,5))

# Generate PDF for all cancers, combined
pdf_ur.epi.comb <- ggplot2::ggplot(ur.epi.comb.perugm3
    ...
    ggplot2::xlim(0,0.002) +
    ggplot2::theme_bw() +
    ggplot2::labs(title = "All cancers in workers"
                  , subtitle="Probability Density Function"
                  , x=xlabel_ur
                  , y = "Density") +
    ggplot2::theme(plot.title=ggplot2::element_text(hjust=0.5, size=8)
                   , plot.subtitle = ggplot2::element_text(hjust=0, size=6)
                   , axis.text = ggplot2::element_text(size=6)
                   , axis.title = ggplot2::element_text(size=8))
...
# Combine plots into one figure
epi_screen <-"Epidemiology Screening-level IUR"
stg <- grid::textGrob(epi_screen, gp=grid::gpar(fontsize=10))
cap1 <- grid::textGrob(generic.caption, gp=grid::gpar(fontsize=8
                                                     , fontface="italic"))
cap2 <- grid::textGrob(epi_caption, gp=grid::gpar(fontsize=8
                                                  , fontface="italic"))
# 1=tg; 2=stg; 3=pdf_ur.epi.lymph; 4=blankPlot; 5=pdf_ur.epi.breast
# 6=pdf_ur.epi.comb; 7=cdf_ur.epi.comb; 8=cap1; 9=cap2
    
fn.PDFCDF.ur.epi.comb <- file.path(outdir,"HistECDF_IURepiAll_screen.png")
p_PDFCDF_ur.epi.comb <- gridExtra::grid.arrange(tg, stg, pdf_ur.epi.lymph
                            , blankPlot, pdf_ur.epi.breast, pdf_ur.epi.comb
                            , cdf_ur.epi.comb, cap1, cap2, ncol=4, nrow=6
                            , layout_matrix = rbind(c(1,1,1,1), c(2,2,2,2)
                                                    , c(3,4,5,4), c(6,4,7,4)
                                                    , c(8,8,8,8), c(9,9,9,9))
                            , heights = c(0.3, 0.2, 3, 3, 0.2, 0.2)
                            , widths = c(3.5,0.1,3.5,0.1))

```

```{r display_ur.epi.comb, out.width="75%"}

ggplot2::ggsave(fn.PDFCDF.ur.epi.comb, p_PDFCDF_ur.epi.comb, height = 7
                , width = 8, dpi=bestdpi)

knitr::include_graphics(path = fn.PDFCDF.ur.epi.comb)
...
# Generate PDF of lymph cancers in adults (refined)
pdf_epi.lymph.adults <- ggplot2::ggplot(ur.epi.lymph.adults
                                        , ggplot2::aes(UnitRisk)) + 
    ggplot2::geom_density(ggplot2::aes(y=ggplot2::after_stat(count/max(count)))
                          , color=pal6[1], size=1) +
    ggplot2::xlim(-0.005,0.006) +
    ggplot2::theme_bw() +
    ggplot2::labs(title="Lymphoid cancers in workers"
                  # , x=xlabel_ur
                  , y = "Density") +
    ggplot2::theme(plot.title = ggplot2::element_text(hjust=0.5, size=8)
                   , axis.text = ggplot2::element_text(size=6)
...
# Generate PDF of breast cancers in adults (refined)
pdf_epi.breast.adults <- ggplot2::ggplot(ur.epi.breast.adults
    ...
    ggplot2::xlim(-0.005,0.006) +
    ggplot2::theme_bw() +
    ggplot2::labs(title="Breast cancer in workers"
                  # , x=xlabel_ur
                  , y = "Density") +
    ggplot2::theme(plot.title = ggplot2::element_text(hjust=0.5, size=8)
                   , axis.text = ggplot2::element_text(size=6)
...
```{r ur.epi.comb.adults, include=FALSE, echo=FALSE}

# Do not use per CKirman
...
pdf_epi.comb.adults.sum <- ggplot2::ggplot(ur.epi.comb.adults
    ...
    ggplot2::geom_density(ggplot2::aes(y=ggplot2::after_stat(count/max(count)))
                          , color=pal6[1], size=1) +
    ggplot2::theme_bw() +
    ggplot2::labs(title="All cancers in workers (lines 1 + 2)"
                  , caption = captionSum
                  , x = xlabel_ur
                  , y = "Count") +
    ggplot2::theme(plot.title = ggplot2::element_text(hjust=0.5, size=10)
...
cdf_epi.comb.adults.sum <- ggplot2::ggplot(ur.epi.comb.adults
                                       , ggplot2::aes(UnitRisk)) +
    ggplot2::stat_ecdf(geom = "point", color = pal6[1]) +
    ggplot2::theme_bw() +
    ggplot2::labs(title="All cancers in workers (lines 1 + 2)"
                  , caption = captionSum
                  , x = xlabel_ur
...
p_PDFCDF_ur.epi.comb2 <- gridExtra::grid.arrange(pdf_epi.lymph.adults
                        , pdf_epi.breast.adults, pdf_epi.comb.adults.sum
                        , cdf_epi.comb.adults.sum, nrow=2, ncol=2
                        , top=groupTitle)
fn.PDFCDF.ur.epi.comb2 <- file.path(outdir,"HistECDF_IURepiAll_lines1and2.png")
ggplot2::ggsave(fn.PDFCDF.ur.epi.comb2, p_PDFCDF_ur.epi.comb2, height=5
                , width=7, dpi=600)

knitr::include_graphics(path = fn.PDFCDF.ur.epi.comb2)
...
```{r ur.epi.comb.adults2, fig.show="hide"}
...
captionFINAL <- paste0("All cancers: Mean="
                     , format(meanIURfinal,digits=3,nsmall=0)
                     , "; Std. Dev.="
                     , format(sdIURfinal,digits=3,nsmall=0))
captionFINAL2 <- paste0("All cancers: Median="
...
# Generate PDF of all cancers (refined)
pdf_epi.comb.adults <- ggplot2::ggplot(ur.epi.comb.adultsFINAL
    ...
    ggplot2::xlim(-0.005,0.006) +
    ggplot2::theme_bw() +
    ggplot2::labs(title="Lymphoid and breast cancers in workers"
                  , x = xlabel_ur
                  , y = "Density") +
    ggplot2::theme(plot.title = ggplot2::element_text(hjust=0.5, size=8)
                   , axis.text = ggplot2::element_text(size=6)
...
# Generate CDF of all cancers (refined)
...
# Draw combined graphic
ref.epi.sub <- "Epidemiology Refined IUR"
stg <- grid::textGrob(ref.epi.sub, gp=grid::gpar(fontsize=10))
cap1 <- grid::textGrob(generic.caption, gp=grid::gpar(fontsize=8
                                                     , fontface="italic"))
cap2 <- grid::textGrob(captionFINAL2, gp=grid::gpar(fontsize=8
                                                     , fontface="italic"))

# 1=tg; 2=stg; 3=pdf_epi.lymph.adults; 4=blankPlot; 5=pdf_epi.breast.adults
# 6=pdf_epi.comb.adults, 7=cdf_epi.comb.adults; 8=cap1; 9=cap2

fn.PDFCDF.ur.epi.comb3 <- file.path(outdir,"HistECDF_IURepiAll_refined.png")
p_PDFCDF_ur.epi.comb3 <- gridExtra::grid.arrange(tg
                                                 , stg
                                                 , pdf_epi.lymph.adults
                                                 , blankPlot
                                                 , pdf_epi.breast.adults
                                                 , pdf_epi.comb.adults
                                                 , cdf_epi.comb.adults
                                                 , cap1
                                                 , cap2
...
# p_PDFCDF_ur.epi.comb3 <- gridExtra::grid.arrange(pdf_epi.lymph.adults
#                         , pdf_epi.breast.adults, pdf_epi.comb.adults
#                         , cdf_epi.comb.adults, nrow=2, ncol=2, top=groupTitle)
# 
# fn.PDFCDF.ur.epi.comb3 <- file.path(outdir,"HistECDF_IURepiAll_refined.png")
# ggplot2::ggsave(fn.PDFCDF.ur.epi.comb3, p_PDFCDF_ur.epi.comb3, height=5
#                 , width=7, dpi=600)
# 
# knitr::include_graphics(path = fn.PDFCDF.ur.epi.comb3)

```

```{r display_ur.epi.comb3, out.width="75%"}

ggplot2::ggsave(fn.PDFCDF.ur.epi.comb3, p_PDFCDF_ur.epi.comb3, height = 7
                , width = 8, dpi=bestdpi)
...
# Generate combined Screening vs Refined PDF graphic
pdf_epicomp <- ggplot2::ggplot(ur.epi.comb
                    , ggplot2::aes(x=UnitRisk
                                   ,y=ggplot2::after_stat(count/max(count))
                    , color=Type)) +
    ggplot2::geom_density() + 
    ggplot2::scale_color_manual(values=c(pal6[1], pal6[4])) +
    ggplot2::xlim(-0.003, 0.005) +
    ggplot2::theme_bw() +
    ggplot2::labs(title="Distribution of uncertainty around the mean"
...
ggplot2::ggsave(fn_epicomp, pdf_epicomp, height=4, width=5, units="in"
                , dpi=bestdpi)
