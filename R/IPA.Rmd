...
pal6 <- viridis::viridis(6) # purple, blue, aqua, seagreen, limegreen, yellow
bestdpi <- 600
xlabel_pod <- expression(paste("Ethylene oxide, point of departure ("
                               , mu, "g/", m^3, ")"))
xlabel_ur <- expression(paste("Ethylene oxide, inhalation unit risk, per ",mu,"g/",m^{3}))
generic.caption <- paste0(format(nsamps, big.mark = ","), " draws")
blankPlot <- ggplot2::ggplot() + ggplot2::geom_blank(ggplot2::aes(1,1)) +
    ggplot2::theme_void()

# Results directory (create if not existing)
outdir <- "C:/Users/ann.lincoln/Documents/ATSDRproject/Rcode/Results"
ifelse(!dir.exists(file.path(outdir))==TRUE
...
$EC_{c} = \frac{CA\left( \frac{ \mu g}{m^3} \right) \times ET \left( \frac {h}{d} \right) \times EF \left( \frac {d}{y} \right) \times ED \left( y \right)}{AT_{c} \left( y \right) \times \left( \frac {365 d}{y} \right) \times \left( \frac {24 h}{d} \right)}$
...
```{r raw_air, echo=FALSE, fig.show=FALSE}

CA <- "ContaminantConcentration_ug_m3"
fnair <- file.path("..", "Data", CA
...
p_hist_Air <- ggplot2::ggplot(df_CA, ggplot2::aes(Adj_EtO)
  ...
  ggplot2::ylim(0,20) +
  ggplot2::theme_bw() +
  ggplot2::labs(x = xlabelCA, y = "Count") +
...
CA_title <- "Histogram of ethylene oxide concentration in air"
CA_subtitle <- "with theoretical density curves"
CA_caption <- hist_caption 
tg=grid::textGrob(CA_title, gp=grid::gpar(fontsize=12, fontface="bold"))
stg=grid::textGrob(CA_subtitle, gp=grid::gpar(fontsize=10))
cap1=grid::textGrob(CA_caption, gp=grid::gpar(fontsize=6))

fn_CA_dist <- file.path(outdir,"CAdistribution.png")
p_CAdist <- gridExtra::grid.arrange(tg, stg, p_hist_Air, p_hist_Air01, blankPlot
                                    , cap1, ncol=4, nrow=4
                                    , layout_matrix = rbind(c(1,1,1,1), c(2,2,2,2)
                                                       , c(3,5,4,5), c(6,6,6,6))
                                    , heights = c(0.2, 0.2, 3, 0.2)
                                    , widths = c(2.3, 0.1, 2.3, 0.1))

ggplot2::ggsave(fn_CA_dist, p_CAdist, height=4, width=5, units="in", dpi=bestdpi)

```


```{r displayCA, out.width="80%"}

knitr::include_graphics(fn_CA_dist)

rm(df_lnorm, df_lnorm2, p_hist_Air01, p_hist_Air, fnair, hist_caption
  , hist_color, hist_labs, xlabel, CA, p_CAdist, fn_CA_dist, CA_caption
  , CA_subtitle, CA_title, cap1)
...
fnED <- file.path("..", "Data", ED, "ExpDuration_2017_2015_2008_summary.xlsx")
...
wd2017 <- file.path("..", "Data", ED, "AHS 2017 National PUF v3.0 SAS")
...
p_histED2017_raw <- ggplot2::ggplot(df_ED2017_summary
  ...
  ggplot2::xlim(0,100) +
  ggplot2::theme_bw() +
  ggplot2::labs(title="Histogram of Exposure Duration"
...
p_cdfED2017_raw <- ggplot2::ggplot(df_ED2017_summary
  ...
  ggplot2::xlim(0,100) +
  ggplot2::theme_bw() +
  ggplot2::labs(title="CDF of years in residence"
...
fn_EDVar_PCDF2017 <- file.path(outdir,"EDvar_PDFCDF_2017.png")
p_EDvar_histCDF2017 <- gridExtra::grid.arrange(p_histED2017_raw, p_cdfED2017_raw
                                               , nrow=1, ncol=2)
ggplot2::ggsave(fn_EDVar_PCDF2017, p_EDvar_histCDF2017, width=5, height=4
                , dpi=bestdpi)
...
wd2015 <- file.path("..", "Data", ED, "AHS 2015 National PUF v3.0 SAS")
...
fn_EDVar_PCDF2015 <- file.path(outdir,"EDvar_PDFCDF_2015.png")
p_EDvar_histCDF2015 <- gridExtra::grid.arrange(p_histED2015_raw, p_cdfED2015_raw
                                               , nrow=1, ncol=2)
ggplot2::ggsave(fn_EDVar_PCDF2015, p_EDvar_histCDF2015, width=5, height=4
                , dpi=bestdpi)
...
wd2007 <- file.path("..", "Data", ED, "AHS 2007 National PUF v1.1 SAS")
...
fn_EDVar_PCDF2007 <- file.path(outdir,"EDvar_PDFCDF_2007.png")
p_EDvar_histCDF2007 <- gridExtra::grid.arrange(p_histED2007_raw, p_cdfED2007_raw
                                               , nrow=1, ncol=2)
ggplot2::ggsave(fn_EDVar_PCDF2007, p_EDvar_histCDF2007, width=5, height=4
                , dpi=bestdpi)
...
p_cdfED_all <- ggplot2::ggplot(df_ED_all_long) +
  ...
  ggplot2::geom_smooth(ggplot2::aes(x=NumYears, y=CumFract_summary)
             ...
             , na.rm = TRUE) +
  ggplot2::scale_color_manual(values = pal4) +
...
fn_cdfEDall <- file.path(outdir, "CompareAHS_RawVsSummary_AllYears.png")
ggplot2::ggsave(fn_cdfEDall, p_cdfED_all, width=7, height=7, dpi=bestdpi)
...
```{r display_AHScompare, out.width="80%"}

knitr::include_graphics(fn_cdfEDall)
...
rm(p_cdfED2007_raw, p_cdfED2015_raw, p_cdfED2017_raw, p_histED2007_raw
  ...
  , df_ED_2007trim, df_ED_2015trim, df_ED_2017trim, df_ED_all
  , df_ED_all_formal, df_ED_all_long, df_ED_raw, df_ExpDur, p_cdfED_all
  , p_EDvar_histCDF2007, p_EDvar_histCDF2015, p_EDvar_histCDF2017
  , EDall_caption, fn_cdfEDall, fn_EDVar_PCDF2007, fn_EDVar_PCDF2015
  , fn_EDVar_PCDF2017)

```


```{r ED_select, out.width="80%", echo=FALSE}

fnStats <- file.path("..", "Data", ED
                     , "SpecificStatsFromAHSDataMultipleYears.xlsx")
...
rm(dfStats, dfStats2, Stats_caption, stats_colnames, fnStats)
...
p_cdfED2017_comp <- ggplot2::ggplot(df_ED2017_sumB
  ...
  ggplot2::geom_smooth(data=df_EDsampleCDF
             ...
             , method = "loess", se=FALSE, color=pal4[3]
             , size = 1) +
  ggplot2::geom_point(color = pal4[1], shape=20) +
  ggplot2::xlim(0,100) +
  ggplot2::theme_bw() +
  ggplot2::labs(subtitle="CDF of number of years in residence"
         , caption = ahs_caption
         , x = "Years", y = "Cumulative Fraction") +
  ggplot2::theme(plot.subtitle = ggplot2::element_text(hjust=0.5, size=8)
          , axis.text = ggplot2::element_text(size=6)
          , axis.title = ggplot2::element_text(size=8)
          , plot.caption = ggplot2::element_text(size=6))
...
p_histED2017_resamp <- ggplot2::ggplot(as.data.frame(df_EDsampHist)
  ...
  ggplot2::xlim(0,100) +
  ggplot2::theme_bw() +
  ggplot2::labs(title="Exposure duration, actual data, adults 18+"
         , subtitle="Histogram of years residing in one house"
         , caption = ""
         , x = "Years", y = "Number of People") +
  ggplot2::theme(plot.title = ggplot2::element_text(hjust=0.5, size=10)
          , plot.subtitle = ggplot2::element_text(hjust=0.5, size=8)
          , axis.text = ggplot2::element_text(size=6)
          , axis.title = ggplot2::element_text(size=8)
          , plot.caption = ggplot2::element_text(hjust=1, size = 6)
          , legend.position = "none")

fn_ED2017var <- file.path(outdir, "ED2017resampDist.png")
p_ED2017var <- gridExtra::grid.arrange(p_histED2017_resamp, p_cdfED2017_comp
                                       , nrow=2, ncol=1)

ggplot2::ggsave(fn_ED2017var, p_ED2017var, width=5, height=4, dpi=bestdpi)
...
rm(df_EDsample, df_EDsampleCDF, df_ED2017_complete, df_ED2017_summary
  ...
  , fnStats, df_ED2017_sumB, ED, fn_ED2017var, p_ED2017var)
...
rm(descED, descEDAdj, df_descED, df_descED, desc_caption, descEDcont)
...
dcomp <- fitdistrplus::cdfcomp(list(ED_fit_gamma, ED_fitweibull)
                ...
                , main="AHS 2017 occupancy (Exposure Duration)"
                , xlab = "Number of years", xlim = c(0, 90)
                , fitcol = c(pal4[1],pal4[3]), fitlty = 1
                , xlegend = "topright", plotstyle = "ggplot"
                , addlegend = TRUE)
dcomp + ggplot2::theme_bw() +
    ggplot2::theme(plot.title = ggplot2::element_text(hjust=0.5, size=8)
          , axis.text = ggplot2::element_text(size=6)
          , axis.title = ggplot2::element_text(size=8))

fn_EDgof <- file.path(outdir,"ED_GoF_CDF.png")
ggplot2::ggsave(fn_EDgof, dcomp, height=4, width=7, units="in", dpi=bestdpi)
...
p_qqED <- ggplot2::ggplot(df_qq, ggplot2::aes(x=theorqg, y=sampq)) +
...
fn_EDqq <- file.path(outdir,"ED_qq.png")
ggplot2::ggsave(fn_EDqq, p_qqED, width=5, height=4, dpi=bestdpi)

rm(ED_fitlognorm, ED_fitweibull, df_gof, gof_ED_gamma, gof_ED_lnorm
  , gof_ED_weib, desc_caption, gof_caption, fn_EDgof, fn_EDqq, ADstargamma
  , ADstarln, ADstarwb, ADstatgamma, ADstatln, ADstatwb, nsamp, sampq
  , theorqg, theorqw, p_qqED, df_qq, dcomp)
...
rm(df_ED_EProb_char, ED_EProb_caption, ED_EProb_headings, gof_EDcaption
   , gof_EDheadings, grandTotalEDProbs, grandTotalPeople, ED_fit_gamma)
...
rm(desc_ETvar, desc_caption, df_descETvar)
...
ET_ddens <- fitdistrplus::denscomp(fit_ET_tnorm
                        , main = NULL
                        , xlab = "Hours per day", xlim = c(0,24)
                        , fitcol = "red", fitlty = 1
                        , plotstyle = "ggplot", addlegend = FALSE)
p_ET_ddens <- ET_ddens + ggplot2::theme_bw() +
    ggplot2::theme(plot.title = ggplot2::element_text(hjust=0, size=8)
          , axis.text = ggplot2::element_text(size=6)
          , axis.title = ggplot2::element_text(size=8)
          , legend.position = "none")
...
ET_dcomp <- fitdistrplus::cdfcomp(fit_ET_tnorm
                        ...
                        , fitcol = "red", fitlty = 1
                        , plotstyle = "ggplot", addlegend = FALSE)
p_ET_dcomp <- ET_dcomp + ggplot2::theme_bw() +
    ggplot2::theme(plot.title = ggplot2::element_text(hjust=0, size=8)
          , axis.text = ggplot2::element_text(size=6)
          , axis.title = ggplot2::element_text(size=8)
          , legend.position = "none")

fn_ETvar <- file.path(outdir,"ETvar_gofplots.png")
ETinfo <- "Truncated normal distribution fit to empirical data"
tg <- grid::textGrob("Distribution of Exposure Time"
                     , gp=grid::gpar(fontsize=10))
stg <- grid::textGrob(ETinfo, gp=grid::gpar(fontsize=8))
p_ETvar_gof <- gridExtra::grid.arrange(tg, stg, p_ET_ddens, p_ET_dcomp
                                       , blankPlot, ncol=4, nrow=3
                                       , layout_matrix = rbind(c(1,1,1,1)
                                                               , c(2,2,2,2)
                                                               ,c(3,5,4,5))
                                       , heights = c(0.2, 0.2, 3.5)
                                       , widths = c(3, 0.5, 3, 0.5))
ggplot2::ggsave(fn_ETvar, p_ETvar_gof, width=7, height=5, dpi=bestdpi)
...
rm(desc_ETvar, df_descETvar, df_gofET, ET_dcomp, ET_ddens, gofETntrunc
   , desc_caption, gof_ETcaption, lbound, ubound, p_ETvar_gof, p_ET_dcomp
   , p_ET_ddens, fn_ETvar, ETinfo, tg, stg)
...
```{r AT_data, echo=FALSE, fig.show="hide"}

wd <- file.path("..", "Data", "AveragingTime_y")
...
p_cdf2007 <- ggplot2::ggplot(df_AT07, ggplot2::aes(x=Age, y=CumFractDying)) +
    ggplot2::geom_smooth(method = "loess", se=FALSE, color=pal6[4]
                         , size = 1) +
    ggplot2::geom_point(color = pal6[2], shape=20) +
    ...
    ggplot2::ylim(0,1) +
    ggplot2::theme_bw() +
    ggplot2::labs(title="ECDF of lifespan (2007 data)"
...
p_cdf2017 <- ggplot2::ggplot(df_AT17, ggplot2::aes(x=Age, y=CumFract)) +
    ggplot2::geom_smooth(method = "loess", se=FALSE, color=pal6[3]
                         , size = 1) +
    ggplot2::geom_point(color = pal6[1], shape=20) +
    ggplot2::geom_vline(ggplot2::aes(xintercept=18), color = "red", size=1) +
    ggplot2::geom_vline(ggplot2::aes(xintercept=85), color = "red", size=1) +
    ggplot2::annotate(geom="text", y=0.80, x=23, label="18 years", color="red"
                      , size = 3) +
    ...
    ggplot2::ylim(0,1) +
    ggplot2::theme_bw() +
    ggplot2::labs(title="ECDF of lifespan (2017 Life Table data)"
...
fn_AT_2011v2017 <- file.path(outdir,"AT_CompDataDistr.png")
p_AT_2011v2017 <- gridExtra::grid.arrange(p_histAT2017, p_cdf2017, p_cdf2007
                                          , nrow=3, ncol=1)
ggplot2::ggsave(fn_AT_2011v2017, p_AT_2011v2017, height = 7, width = 7
                , units = "in", dpi = bestdpi)

```

```{r display_AT2011v2017, out.width="80%"}

knitr::include_graphics(fn_AT_2011v2017)

rm(fn2007, fn2017, grandTotalPeople, hist_caption, AT17_caption, fn_ccompAT
   , fn_dcompAT, fn_AT_2011v2017)
...
```{r gof_AT, echo=FALSE, fig.show="hide"}
...
# Graph results
dcompAT <- fitdistrplus::denscomp(list(fitAT_weib, fitAT_gamma)
                , main = NULL
                , xlab = "Number of years", xlim = c(0, 100)
                , fitcol = c(pal6[2],pal6[4])
                , fitlty = 1, xlegend = "topright", plotstyle = "ggplot"
                , addlegend = FALSE)
dcompAT <- dcompAT + ggplot2::theme_bw() +
    ggplot2::geom_vline(xintercept = 18, color="red") +
    ggplot2::geom_vline(xintercept = 85, color="red") +
    ggplot2::annotate(geom="text",label="18",x=21,y=0.035,color="red",size=3) +
    ggplot2::annotate(geom="text",label="85",x=82,y=0.035,color="red",size=3) +
    ggplot2::theme(plot.title =ggplot2::element_blank()
                   , axis.text = ggplot2::element_text(size=6)
                   , axis.title = ggplot2::element_text(size=8)
                   , legend.position = "none")

fn_dcompAT <- file.path(outdir, "AT_HistGoF.png")
ggplot2::ggsave(fn_dcompAT, dcompAT, height=4, width=7, units="in"
                , dpi=bestdpi)
...
ccompAT <- fitdistrplus::cdfcomp(list(fitAT_weib #, fitAT_nbinom
                ...
                , fitlty = 1, xlegend = "topright", plotstyle = "ggplot"
                , addlegend = FALSE)
ccompAT <- ccompAT + ggplot2::theme_bw() +
    ggplot2::geom_vline(xintercept = 18, color="red") +
    ggplot2::geom_vline(xintercept = 85, color="red") +
    ggplot2::annotate(geom="text",label="18",x=21,y=0.875,color="red",size=3) +
    ggplot2::annotate(geom="text",label="85",x=82,y=0.875,color="red",size=3) +
    ggplot2::theme(plot.title =ggplot2::element_blank()
                   , axis.text = ggplot2::element_text(size=6)
                   , axis.title = ggplot2::element_text(size=8)
                   , legend.position="none")

fn_ccompAT <- file.path(outdir, "AT_CDGoF.png")
ggplot2::ggsave(fn_ccompAT, ccompAT, height=4, width=7, units="in"
                , dpi=bestdpi)

tg <- grid::textGrob("Averaging time goodness of fit curves"
                     , gp=grid::gpar(fontsize=10))
stg <- grid::textGrob("2017 Life table data"
                     , gp=grid::gpar(fontsize=8))
caption <- grid::textGrob("Purple line is Weibull distribution; green line is gamma", gp=grid::gpar(fontsize=6))

fn_AT_PCDFs <- file.path(outdir, "AT_gof.png")
p_AT_PCDFs <- gridExtra::grid.arrange(tg, dcompAT, blankPlot, ccompAT, stg
                                      , caption, ncol=4, nrow=4
                                      , layout_matrix = rbind(c(1,1,1,1)
                                                              , c(5,5,5,5)
                                                              , c(2,3,4,3)
                                                              , c(6,6,6,6))
                            , heights = c(0.2, 0.2, 4.3, 0.2)
                            , widths = c(3,0.1,3,0.1))
ggplot2::ggsave(fn_AT_PCDFs, p_AT_PCDFs, width=7, height=5, units="in"
                , dpi=bestdpi)

```

```{r display_ATgof, out.width="80%"}

knitr::include_graphics(fn_AT_PCDFs)
...
rm(ccompAT, dcompAT, descAT17, descAT17disc, df_AT07, df_AT17, df_gof
   ...
   , fn_AT_PCDFs, fn_ccompAT, fn_dcompAT, p_AT_2011v2017, p_AT_PCDFs
   , caption)
...
```{r CA_uncertainty, echo=FALSE, fig.show="hide"}
...
# Create graph comparing theoretical and empirical CDFs w/95% CI
fn_pCAunc <- file.path(outdir,"CA_gofCDFwith95CI.png")
fitdistrplus::CIcdfplot(boot_CA, "quantile", CI.col=pal6[4], fitcol=pal6[4]
                        , datapch = 1, datacol = pal6[2], do.points = FALSE
                        , main = "ECDF and theoretical CDF with 95% CI"
...
rm(ATUnc_caption, dfATUncert, n, alpha, t_critical_value, AT_CIstudentst
   , df_ATsample, df_AT_adult, df_ATvar)
...
CAvar <- mcstoc(rlnorm, type="V", meanlog=-1.716, sdlog=1.455, nsv=niterv
                , seed=seed, lhs=TRUE)
ETvar <- mcstoc(rnorm, type="V", mean=1084/60, sd=311/60, nsv=niterv
                , seed=seed, rtrunc=TRUE, linf=4, lsup=24, lhs=TRUE)
...
for (n in 1:length(rnames)) {
    ...
    figfn <- file.path(outdir,paste0(param,"_1D.png"))
    ...
    # Graph title, subtitle, captions (for risk)
    tg <- grid::textGrob(paste0("Histogram and CDF of ", param)
                     , gp=grid::gpar(fontsize=10, fontface="bold"))
    stgVar <- grid::textGrob(paste0("n (variability)="
                                    ,format(niterv, big.mark = ",")
                                    ,"; Central tendency IUR")
                             , gp=grid::gpar(fontsize=8))
    captionRisk50 <- grid::textGrob(paste("Red line represents risk level = 1E-06", paste0("Blue line represents median risk level (", Param50, ")"), paste0("Green line represents 95th percentile risk level (", Param95,")"), sep="\n"), gp=grid::gpar(fontsize=6, fontface="italic"))

    captionRisk95 <- grid::textGrob(paste("Red line represents risk level = 1E-06", paste0("Blue line represents median risk level (", Param50, ")"), paste0("Green line represents 95th percentile risk level (", Param95,")"), paste0("Orange line is the maximum point risk estimate (", Risk_max_iur95, ")"), paste0("Gray line is RME point risk estimate (", Risk_95ucl_iur95, ")"), sep="\n"), gp=grid::gpar(fontsize=6, fontface="italic"))
    
    captionElse <- grid::textGrob(paste(paste0("Blue line represents median parameter value (", Param50, ")"), paste0("Green line represents 95th percentile parameter level (", Param95,")"), sep="\n"), gp=grid::gpar(fontsize=6, fontface="italic"))
    ...
    if (param=="Risk50") {
        ...
        phist <- ggplot2::ggplot(df2plot, ggplot2::aes(Parameter)) +
            ...
            ggplot2::theme_bw() +
            ggplot2::labs(x=xlabel, y="Frequency") +
            ggplot2::theme(axis.text = ggplot2::element_text(size=6)
        ...
        pcdf <- ggplot2::ggplot(df2plot, ggplot2::aes(Parameter)) +
            ...
            ggplot2::xlim(xmin, xmax) +
            ggplot2::theme_bw() +
            ggplot2::labs(x=xlabel, y="Probability") +
            ggplot2::theme(axis.text = ggplot2::element_text(size=6)
                    , axis.title = ggplot2::element_text(size = 6)
                    , legend.position = "none")
        
        ggplot2::ggsave(figfn
                    , gridExtra::grid.arrange(tg, stgVar, phist, blankPlot
                                , pcdf, captionRisk50, ncol=4, nrow=4
                                , layout_matrix=rbind(c(1,1,1,1)
                                                      , c(2,2,2,2)
                                                      , c(3,4,5,4)
                                                      , c(6,6,6,6))
                                , widths=c(3.2,0.3,3.2,0.3)
                                , heights=c(0.2, 0.2, 4, 0.6))
        ...
        pcdf <- ggplot2::ggplot(df2plot, ggplot2::aes(Parameter)) +
            ...
            ggplot2::geom_vline(xintercept = Risk_max_iur95, color="orange"
                                , size=1, linetype="dashed") +
            ggplot2::geom_vline(xintercept = Risk_95ucl_iur95
                                    , color="gray"
                                , size=1, linetype="dashed") +
            ggplot2::xlim(xmin, xmax) +
            ggplot2::theme_bw() +
        ...
        ggplot2::ggsave(figfn
                    , gridExtra::grid.arrange(tg, stgVar, phist, blankPlot
                                , pcdf, captionRisk95, ncol=4, nrow=4
                                , layout_matrix=rbind(c(1,1,1,1)
                                                      , c(2,2,2,2)
                                                      , c(3,4,5,4)
                                                      , c(6,6,6,6))
        ...
        ggplot2::ggsave(figfn
                    , gridExtra::grid.arrange(tg, stgVar, phist, blankPlot
                                , pcdf, captionElse, ncol=4, nrow=4
                                , layout_matrix=rbind(c(1,1,1,1)
                                                      , c(2,2,2,2)
                                                      , c(3,4,5,4)
                                                      , c(6,6,6,6))
...
ETvu <- mcstoc(rnorm, type="VU", mean=ET_mean, sd=ET_sd, nsv=niterv
               , nsu=niteru, seed=seed, rtrunc, linf=4, lsup=24, lhs=TRUE)
...
pCDF_mc2dRisk <- plot.mc(Risk2d$Risk, draw=FALSE, paint=FALSE)
dfRiskECDFdata <- data.frame(unmc(pCDF_mc2dRisk))
...
Risk_reg <- 1.0e-6          # Regulatory criterion
...
pcdf_risk_med95CI <- ggplot2::ggplot(dfRiskECDFdata, ggplot2::aes(x=median)) +
    ...
    ggplot2::geom_vline(xintercept = Risk_reg, color="red", size=1
                        , linetype="dashed") +
    ggplot2::geom_vline(xintercept = Risk_max_iur95, color="orange"
                        , size=1, linetype="dashed") +
    ggplot2::geom_vline(xintercept = Risk_95ucl_iur95, color="purple"
                        , size=1, linetype="dashed") +
    ggplot2::xlim(c(-1e-5,0.01)) +
    ggplot2::theme_bw() +
...
fnMedRisk2d <- file.path(outdir,"RiskMedian95CI_2D.png")
...
fnMedRisk2d <- file.path(outdir,"RiskMedianIQR_2D.png")
ggplot2::ggsave(fnMedRisk2d, pcdf_risk_medIQR, width=7, height=5, units="in")

figfn <- file.path(outdir,"RiskMedian_BothCDFs.png")
ggplot2::ggsave(figfn
...
fighist <- file.path(outdir, "MedRiskVar_UncHist.png")
...
```{r cleanup2, echo=FALSE, include=FALSE}

rm(boot_CA, boot_ET, dcomp, descEDcont, df_AT_EProb, df_AT_EProb_adult
   , df_CA_vu, df_ED_EProb, df_ED_temp, df_ET_vu, df_IURunc
   , df_qq, df_xlims, dfVarOnly, ED_fit_gamma, p_AT_2011v2017
   , pcdf_risk_medIQR, pcdf_risk_med95CI, phistMedRiskUnc, RiskMedUnc
   , ADstargamma, ADstarln, ADstarwb, ADstatgamma, ADstatln, ADstatwb
   , cdfTitleMed95CI, cdfTitleMedIQR, figfn, figfnls, fighist
   , fn_AT_2011v2017, fn_ccompAT, fn_cdfEDall, fn_dcompAT, fn_EDgof
   , fn_pCAunc, fnMedRisk2d, labels, mc2d_CACaption, mc2d_ETCaption
   , mc2d_RiskCaption, ParamMedian, ParamMedianLCL, ParamMedianUCL
   , riskhistcapt, sampq, subtitle, theorqg, theorqw)

```


# Jim's question re % population

Q: Given observed variability of risk in the population, what percent will experience <1e-6 risk? What is the uncertainty in the estimate of the percent population?

```{r pct_reglimit}

# JD method
makeCategory <- function(x,q) {
    z <- quantile(x,q)
    data.frame(pop = q
           , "<1E-7" = z <1E-7
...
# ARL_method -- extract dataframe
dfVUrisk <- unmc(Risk2d$Risk)
dfVUrisk2 <- as.data.frame(dfVUrisk)
# This applies the function down the column (in other words, is like a total at the bottom of the dataframe). It is putting each column (Uncertainty) into a category based on the quantile (variability) in which that value falls.
byCol <- sapply(c(0.25,0.5, 0.75, 0.9)
             , function(x) apply(dfVUrisk, 2, makeCategory, x))
# This applies the fuction across the rows. It is putting each row (variability) into a category based on the quantile (uncertainty) in which that value falls.
byRow <- lapply(c(0.25,0.5, 0.75, 0.9)
             , function(x) apply(dfVUrisk, 1, makeCategory, x))

# JD method -- work on risk object itself
xx <- sapply(c(0.25,0.5, 0.75, 0.9)
             , function(x) apply(Risk2d$Risk, 2, makeCategory, x))
zz <- dplyr::bind_rows(xx)
zz2 <- dplyr::bind_rows(byCol)

JDpopCats <- zz2 %>% 
    dplyr::group_by(pop) %>% 
    dplyr::summarise_all(mean) %>%
    dplyr::mutate(pop = paste("Variability:", paste0(100 * pop, "%"))) %>%
    ...
    dplyr::mutate(name = factor(name
                         , levels = c("<1E-7"
                                      ...
                                      ,"1E-4 to <1E-3"
                                      ,">1E-3")))
p_JDpopCats <- ggplot2::ggplot(JDpopCats
                               , ggplot2::aes(x=name, y=value
                                              , col=name, fill=name)) + 
    ggplot2::geom_col() + 
    ggplot2::scale_color_manual(values=pal6[1:6]) +
    ggplot2::scale_fill_manual(values=pal6[1:6]) +
    ggplot2::facet_wrap(~pop) + 
    ggplot2::theme_bw() +
    ggplot2::labs(title="Confidence in potential cancer risk categories"
                   , subtitle="Example population based on variability"
                   , y="Probability", x="Cancer Risk Range") +
    ggplot2::theme(plot.title=ggplot2::element_text(hjust=0.5, size=12)
                   , plot.subtitle = ggplot2::element_text(hjust=0.5, size=10)
                   , axis.title = ggplot2::element_text(size=10)
                   , axis.text = ggplot2::element_text(size=8, angle=45
                                                       , hjust=1)
                   , legend.position = "none")

fn_popRiskbyCat <- file.path(outdir,"ConfidenceRiskPopulationSegment.png")
ggplot2::ggsave(fn_popRiskbyCat, p_JDpopCats, width=8, height=6, units="in"
                , dpi=bestdpi)

knitr::include_graphics(fn_popRiskbyCat)

#Obtain statistics of uncertainty for each sample of variability in risk
RiskStatsrowwise <- apply(dfVUrisk,1, function(x) c(mean=mean(x)
                                         , median=quantile(x, probs=0.5)
                                         , LCL2.5=quantile(x, probs=0.0275)
                                         , UCL97.5=quantile(x, probs=0.975)))
RiskStatsrowwise <- as.data.frame(t(RiskStatsrowwise))
colnames(RiskStatsrowwise) <- c("mean", "median", "LCL", "UCL")

# Generate curves as functions to find y for any given x
f_mean <- stats::ecdf(RiskStatsrowwise$mean)
f_median <- stats::ecdf(RiskStatsrowwise$median)
f_LCL <- stats::ecdf(RiskStatsrowwise$LCL)
f_UCL <- stats::ecdf(RiskStatsrowwise$UCL)
x=5e-5
y_mean=f_mean(x)
y_median=f_median(x)
y_LCL=f_LCL(x)
